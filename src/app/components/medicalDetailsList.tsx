...
const MedicalList: React.FC = () => {
  ...
  const fetchResources = async () => {
    try {
      const response = await axios.get('https://sky-nova-8ccaddc754ce.herokuapp.com/medicalDetails/viewMedicalDetails', { withCredentials: true });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setMedicalDetails(medicalDetails.filter(medicalDetails => medicalDetails._id !== id));
    } catch (error) {
          ...
          <tr>
            <th className="py-6 px-4 border-b">
              <input
                ...
                onChange={() => setSelectedResources(selectedResources.length === medicalDetails.length ? [] : medicalDetails.map(medicalDetail => medicalDetail._id))}
              />
            </th>
            {['username','height', 'weight', 'eyesight', 'medicalReport,Actions'].map(header => (
              <th
                key={header}
              ...
              >
                <div className="flex items-center justify-center">
                  {header}
                  {(header === 'Title' || header === 'Type' || header === 'Description') && renderArrow(header.toLowerCase() as keyof medicalDetails)}
                </div>
              </th>
            ))}
          </tr>
  ...
  {currentResources.length > 0 ? (
    currentResources.map(resource => (
      <tr key={resource._id} className="border-b">
        ...
        <td className="py-2 px-4 text-center">{resource.height}</td>
        <td className="py-2 px-4 text-center">{resource.weight}</td>

...
const ManageNonverbalQuizzes: React.FC<ManageQuizzesProps> = ({ onAddQuiz }) => {
  ...
  useEffect(() => {
    const fetchQuizzes = async () => {
      try {
        const response = await fetch('http://localhost:4000/nonVerbalQuizzes/viewNonVerbalQuizzes');
        if (response.ok) {
  ...
  const handleQuizClick = async (quizId: string) => {
    try {
      const response = await fetch(`http://localhost:4000/nonVerbalQuizzes/viewNonVerbalQuiz/${quizId}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteQuiz = async (quizId: string) => {
    try {
      const response = await fetch(`http://localhost:4000/nonVerbalQuizzes/deleteNonVerbalQuiz/${quizId}`, {
        method: 'DELETE',
      });
      if (response.ok) {

...
const VerbalQuizForm = () => {
  ...
  const handleCreateQuiz = async (title: string, description: string) => {
    ...
    try {
      const response = await fetch('http://192.168.18.26:3000/verbalQuizzes/createVerbalQuiz', {
        method: 'POST',
        headers: {
  ...
  const handleAddQuestion = async (question: Question) => {
    ...
    try {
      const response = await fetch('http://192.168.18.26:3000/verbalQuestions/createVerbalQuestion', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },

...
const QuizEditSimple = ({ title }: { title: string }
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      try {
        const response = await fetch(`https://sky-nova-8ccaddc754ce.herokuapp.com/quizzes/viewQuiz/${title}`,{credentials:'include'});
        if (response.ok) {
  ...
  const handleSaveQuiz = async () => {
    try {
      const response = await fetch(`https://sky-nova-8ccaddc754ce.herokuapp.com/quizzes/updateQuiz/${title}`, {
        method: 'PATCH',
        headers: {

...
const NotificationsList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await fetch('http://192.168.18.54:3000/notifications/viewNotifications');
                if (!response.ok) throw new Error('Failed to fetch notifications');
                const data = await response.json();
                const mappedNotifications: Notification[] = data.map((notification: any) => ({
                    id: notification._id,
    ...
    const handleDelete = async (id: string) => {
        try {
            const response = await fetch(`http://192.168.18.54:3000/notifications/deleteNotification/${id}`, {
                method: 'DELETE',
            });
            if (!response.ok) throw new Error('Failed to delete notification');
            setNotifications(notifications.filter((notification) => notification.id !== id));

...
//       },
//       body: JSON.stringify({ type: 'verbalQuiz', description: 'Verbal Quiz Result',answers, marks: resultScore , quizId: quiz?._id, }),
//       credentials: 'include',
//     });
...
const QuizAttempt: React.FC<QuizAttemptProps> = ({ quizId, goBack, goToNextStep }) => {
  ...
  const quizIddd = quizId || params.id;
    const [quiz, setQuiz] = useState<QuizDetail | null>(null);
    const [currentQuestion, setCurrentQuestion] = useState(0);
    const [selectedOption, setSelectedOption] = useState<string | null>(null);
    const [answers, setAnswers] = useState<string[]>([]);
    ...
    useEffect(() => {
        const fetchQuiz = async () => {
            try {
                const response = await fetch(`https://sky-nova-8ccaddc754ce.herokuapp.com/verbalQuizzes/viewVerbalQuiz/${quizIddd}`, {
                    credentials: 'include',
                });
                const data = await response.json();
                setQuiz(data);
    ...
    const handleSubmit = async () => {
        ...
        setQuizFinished(true);
        setShowCompletionScreen(true);
        goToNextStep(); // Call goToNextStep here upon successful submission
    };
    ...
    if (showCompletionScreen && score !== null) {
        ...
        return (
            <div className="bg-gray-900 min-h-screen p-8 flex flex-col justify-center items-center text-white">
                ...
                <button
                    className="bg-blue-500 px-4 py-2 mt-4 rounded-lg"
                    onClick={() => router.push(`/userRender/verbal/${quizId}/result`)}
                >
                    View Detailed Results
                </button>
                <button
